...
Windowsインストーラーは未署名です。macOS 0.1.3以降はPilot Max専用の固定証明書で署名し、更新時の権限判定用の識別条件を維持します。AppleのDeveloper ID署名・公証は行っていません。0.1.2以前からの初回更新では権限の再許可が必要になる場合があります。更新ファイルには、これとは別に検証用の署名を付けます。
